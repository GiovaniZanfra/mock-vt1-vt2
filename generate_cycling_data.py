# Regenerate datasets adding 'gender' column and save updated CSVs + script.
import numpy as np
import pandas as pd
import os
...
def make_dataset(n_samples=800, subject_pool=120, start_idx=0):
    ids = np.arange(start_idx, start_idx + n_samples)
    sid = np.random.randint(1, subject_pool+1, size=n_samples)  # subject id
    # gender: mostly M/F, small 'O' category
    gender = np.random.choice(['M','F','O'], size=n_samples, p=[0.7, 0.29, 0.01])
    age = np.random.randint(18, 66, size=n_samples)  # years
    height = np.random.randint(155, 196, size=n_samples)  # cm
    weight = np.round(np.random.normal(loc=75 - (age-35)*0.1, scale=8, size=n_samples),1)  # kg, slight age effect
...
train = make_dataset(n_samples=800, subject_pool=140, start_idx=0)
test = make_dataset(n_samples=200, subject_pool=140, start_idx=800)

train_path = os.path.join('cycling_train.csv')
test_path = os.path.join('cycling_test.csv')
script_path = os.path.join('generate_cycling_data_with_gender.py')
...
# show previews and simple summary counts for gender
from caas_jupyter_tools import display_dataframe_to_user
display_dataframe_to_user("cycling_train_preview_with_gender", train.head(10))
display_dataframe_to_user("gender_counts_train", train['gender'].value_counts().reset_index().rename(columns={'index':'gender','gender':'count'}))

train_path, test_path, script_path
